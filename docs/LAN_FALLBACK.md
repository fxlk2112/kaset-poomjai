...
The listener binds the Pi's inspected LAN address and accepts only that connected subnet. A dedicated alias service uses the existing Avahi daemon with `--no-reverse --no-fail`; the machine hostname and reverse record are unchanged. This installation assumes the Pi retains that address. If DHCP changes it, re-check the new network and deliberately update protected configuration and restart the LAN/alias services; do not broaden the listener. No router, WAN, public tunnel or Windows firewall settings are changed. The [Avahi flags](https://github.com/avahi/avahi/blob/master/man/avahi-publish.1.xml.in) avoid reverse-record collisions and reconnect to its daemon.
...
2. Prepare the dedicated Linux user, isolated pinned Flask/Gunicorn venv, `avahi-utils`, protected certificate/network configuration and owner verifier. Do not overwrite existing config on drift. Provisioning prints public certificates only. Gunicorn's Python config must keep private helper variables prefixed with `_` to avoid setting-name collisions. Run `--check-config` with the service user's groups, never as root against the live session database.
...
4. Transfer the reviewed tar to a dedicated `/tmp/sucha-lan-*` directory. Run `scripts/lan/install.py --source <candidate-directory> --expected-agent-sha <verified-old-sha>`. It checks the old agent, unused port, no existing service/name and actual all-off readback. It keeps the old agent under `/var/lib/sucha-farm-lan-install`, installs versioned assets, a scoped service drop-in, the weather timer and alias service. Log only safe result codes/version hashes.
