...
export function harness() {
  ...
  const db = { prepare(query) { return { bind(...args) { return { async first() { return sql.prepare(query).get(...args) || null; }, async run() { const r = sql.prepare(query).run(...args); return { meta: { changes: Number(r.changes) } }; } }; } }; }, async batch(statements) { sql.exec("BEGIN"); try { const results = []; for (const s of statements) results.push(await s.run()); sql.exec("COMMIT"); return results; } catch (e) { sql.exec("ROLLBACK"); throw e; } } };
  const env = { MONITOR_DB: db, RELAY_BENCH_ENABLED: "true" };
  const instance = randomUUID();
  const snapshot = () => ({ observed_at: new Date().toISOString(), fault: "NONE", modules: ["RELAY_A", "RELAY_B"].map(id => ({ id, online: true, identity_verified: true, crc_valid: true, mode_verified: true, relay_status: Array(8).fill(false), digital_inputs: Array(8).fill(false) })) });
