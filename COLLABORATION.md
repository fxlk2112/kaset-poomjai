...
พี่ปิ๊กให้ `APPROVE_FARMULTIMATE_DEV_SETUP_ONCE` เมื่อ `2026-09-05` สำหรับงานพัฒนาในขอบเขตนี้.

1. Consolidate และ review งานที่ยังอยู่บน `sucha/sensor-phase1-local` โดยรักษา raw worktree ไว้.
2. Scan staged files ว่าไม่มี secret, farm coordinates, device identity หรือ private runtime data.
3. Push integration baseline ไป feature branch และสร้าง `develop` จาก commit ที่ตรวจแล้ว.
4. หลัง setup นี้ owner แต่ละคน push feature branch ของตนและเปิด PR เข้า `develop` ได้โดยไม่ต้องขอ feature-push approval ซ้ำ.
...
สิทธิ์นี้ไม่อนุมัติ `master`, production deploy, secret/config write, การลบ branch/data หรือ hardware action. Production ยังต้องใช้ `APPROVE_PRODUCTION_DEPLOY` แยกต่างหาก.
...
จากนั้น stage เฉพาะไฟล์ใน scope, commit ขนาดเล็ก และ push เฉพาะ feature branch ของ owner ตามสิทธิ์ setup ครั้งเดียว.

## One App / One Domain

```text
Browser -> FARMULTIMATE public origin -> /api -> Pages Function
                                             -> Service Binding -> Worker -> D1/R2
```

หน้า Farm และ Commerce อยู่ใน repository และ release เดียวกัน. Browser ไม่ต้องรู้ public Worker URL และทุก deployed page ต้องใช้ `/api` ของ origin เดียวกัน. ดู `docs/SAME_ORIGIN_API.md`.
...
ขอบเขตฟีเจอร์ถาวรและ shared integration files อยู่ใน `docs/FEATURE_OWNERSHIP.md`.
