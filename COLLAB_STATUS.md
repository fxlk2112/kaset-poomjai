...
- V2 status: `HARDWARE_AND_SCHEMA_PASS / PUBLICATION_IN_PROGRESS`. 114 JavaScript tests, eight Python tests and nine relay messages pass; phone/tablet/desktop QA passes. Actual Pi test read all 16 channels ON simultaneously, independent OFF preserved other channels, all hardware timers and global OFF passed. The schema migration preserves all four audit rows. The bench service is temporarily stopped for coordinated publication; all outputs OFF.
