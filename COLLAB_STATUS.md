...
- Status: `HARDWARE_BENCH_PASS / CLOUD_SCHEMA_VERIFIED`. Pick granted `APPROVE_RELAY_BENCH_CLOUD` on 2026-09-06. At 21:02 Bangkok, both new tables and both indexes exactly match the prepared schema on independent readback. Existing business tables remain untouched. Frontend publication and isolated Pi activation follow under the same approval. All 16 unloaded channels previously passed ON, autonomous five-second OFF and early OFF; latest preflight confirms all OFF with the bench unit still disabled.
